...
export async function renderPlay(root: HTMLElement): Promise<void> {
  ...
  async function startMatch(): Promise<void> {
    ...
    // 每局给 AI 一个不同的种子 → 随机抽一种打法风格（同图每局不一样）。无任何经济/起手作弊：
    // 双方起始资源一致，AI 全靠策略（攒波出击 + 防御投入）取胜。
    const ai = new SimpleAI(AI_ID, difficulty, (Date.now() ^ (AI_ID * 2654435761)) >>> 0);
    // 敌情简报：开局揭示对手打法风格 + 难度（让 AI 显得有性格）
    ...
    // 始终暴露对局驱动接口（AllinONE 平台 effectCode 可通过 __ra2allinone.emit 注入命令，
    // 此处暴露 step 供平台主动推进模拟 tick，如超时补偿等场景）
    ...
    // DEV 模式保留 __ra2play 名称不变（调试工具兼容）
  }
