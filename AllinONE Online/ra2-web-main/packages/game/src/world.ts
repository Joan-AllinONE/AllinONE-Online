...
export type Command =
  ...
  // ──── AllinONE 平台 effectCode 专用命令（走游戏自身命令管线，由 applyCommands 处理）────
  | { kind: 'addCash'; owner: number; amount: number }
  | { kind: 'repairAll'; owner: number }
  | { kind: 'powerSurge'; owner: number; durationTicks: number }
  | { kind: 'revealMap'; owner: number; durationTicks: number }
  | { kind: 'spawnAlly'; owner: number; typeId: string; cellX: number; cellY: number };
...
export class World {
  ...
  /** AllinONE effectCode: 电力激增剩余 tick（owner → 剩余帧数）。 */
  _powerSurgeTimers = new Map<number, number>();
  /** AllinONE effectCode: 全图视野剩余 tick（owner → 剩余帧数）。 */
  _revealTimers = new Map<number, number>();
    ...
    for (const cmd of commands) {
      switch (cmd.kind) {
        ...
        case 'stop':
          ...
          break;
        // ──── AllinONE 平台 effectCode 专用命令处理 ────
        case 'addCash': {
          const p = this.players.get(cmd.owner);
          if (p && !p.defeated) p.credits += cmd.amount;
          break;
        }
        case 'repairAll': {
          for (const e of this.entities.values()) {
            if (e.owner === cmd.owner) e.hp = e.maxHp;
          }
          break;
        }
        case 'powerSurge': {
          // 电力激增：指定玩家的发电建筑在 durationTicks 内产出翻倍
          // 由 stepPower 中的 _powerSurgeTimers 控制
          this._powerSurgeTimers.set(cmd.owner, cmd.durationTicks);
          break;
        }
        case 'revealMap': {
          // 全图视野：标记指定玩家在 durationTicks 内临时无视迷雾
          // 由 WorldRenderer 的 cellVisible 检查此标记
          this._revealTimers.set(cmd.owner, cmd.durationTicks);
          break;
        }
        case 'spawnAlly': {
          // 友军增援：在指定位置直接生成单位（不需要生产建筑前置）
          const type = this.rules.units.get(cmd.typeId);
          if (type && !type.domain?.includes('building')) {
            this.makeEntity(cmd.owner, type, cellToLepton(cmd.cellX), cellToLepton(cmd.cellY));
          }
          break;
        }
      }
  ...
  private stepPower(): void {
    ...
    for (const e of this.entities.values()) {
      ...
      // 受损建筑按血量比例发电（红警2 风格）
      if (b.power > 0) {
        const surge = this._powerSurgeTimers.get(e.owner) ?? 0;
        const multiplier = surge > 0 ? 2 : 1;
        p.powerProduced += Math.floor((b.power * e.hp * multiplier) / e.maxHp);
      } else {
        p.powerDrained += -b.power;
      }
    }
    // 递减电力激增计时器
    for (const [owner, ticks] of this._powerSurgeTimers) {
      if (ticks <= 1) this._powerSurgeTimers.delete(owner);
      else this._powerSurgeTimers.set(owner, ticks - 1);
    }
  }
  ...
  step(): void {
    ...
    this.stepRepair();
    // 递减全图视野计时器
    for (const [owner, ticks] of this._revealTimers) {
      if (ticks <= 1) this._revealTimers.delete(owner);
      else this._revealTimers.set(owner, ticks - 1);
    }
    for (const e of this.entities.values()) {
      const type = this.rules.units.get(e.typeId);
      if (!type) continue;
