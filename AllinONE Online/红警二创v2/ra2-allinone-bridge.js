...
(function() {
  ...
  function registerDynamicEffect(effectName, effectCodeStr) {
    ...
    try {
      ...
      // 创建沙箱函数，注入 game / getGame 桥接对象
      // 兼容两种写法：① function(params){...} 函数表达式 ② 直接使用 game 的语句体
      var sandboxFn = (function () {
        if (/^function\s*\(/.test(effectCodeStr.trim())) {
          return new Function('game', 'Math', 'JSON', 'console', 'getGame',
            'return (' + effectCodeStr.trim() + ')(game,Math,JSON,console);');
        }
        return new Function('game', 'Math', 'JSON', 'console', 'getGame', effectCodeStr);
      })();
      var wrappedHandler = function(params) {
        try {
          var g = getGame();
          var result = sandboxFn(g || {}, Math, JSON, console, function () { return g || {}; });
