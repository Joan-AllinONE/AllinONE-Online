...
(function() {
  ...
  function registerDynamicEffect(effectName, effectCodeStr) {
    ...
    try {
      // 安全校验：禁止访问全局危险对象
      if (/document\.(cookie|domain)|eval\(|Function\(|XMLHttpRequest|fetch\(|import\(/i.test(effectCodeStr)) {
  ...
  // 通道 2: DOM 事件（平台内页 redeem）
  function handleRedeemedItem(detail) {
    if (!detail) return;
    var voucherData = detail.voucher || detail;
    var detailData = detail.detail || detail;
    var key = detail.key || voucherData.key || '';
    var realEffect = voucherData.effect || detailData.effect || '';
    if (!realEffect) return;
    customPowerUps.push({
      id: key || ('ugc_' + Date.now()),
      name: voucherData.name || detailData.itemName || '道具',
      effect: realEffect,
      params: voucherData.params || detailData.params || {},
      effectCode: voucherData.effectCode || null,
      icon: voucherData.icon || '⚡',
    });
    renderCustomPowerUps();
    showToast('🎁 获得道具: ' + (detailData.itemName || realEffect), 'success');
  }
