...
const CACHE_NAME = 'allinone-gamefiles-v3';
...
// 后端命中时：若是入口 HTML 且有注入扩展 → 注入后返回新的 Response（不污染原始缓存语义）。
// ⚠️ 幂等：后端/云函数已在文件分发层注入（内容含注入标记）则直接透传，避免重复注入。
async function maybeInjectEntryHtml(gameId, filePath, resp) {
  const mime = getMimeType(filePath);
  ...
  let out = text;
  const injected = applyInjectionsToHtml(out, meta.injections);
  ...
  console.log('[SW] 注入扩展到入口 HTML:', filePath, 'injections:', meta.injections.length, 'loader:', meta.contentSopEnabled);
...
async function handleGameFileRequest(req) {
  ...
  console.log('[SW] 游戏文件请求:', parsed.gameId, parsed.filePath);
  ...
  try {
    ...
    if (resp && resp.ok) {
      // 方案 E：入口 HTML 注入扩展（仅当游戏详情存在 injections）
      const out = await maybeInjectEntryHtml(parsed.gameId, parsed.filePath, resp);
